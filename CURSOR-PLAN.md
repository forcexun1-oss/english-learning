...
- "Generate Audio" button — calls `/api/generate-audio/[date]` if `.aiff` does not exist yet

**Data:** Serve audio file via Next.js API route; read `.txt` file

**API route: `GET /api/audio/[date]`**

```typescript
// Stream LEARNING_DIR/audio/YYYY-MM-DD.aiff as audio/aiff
// Return 404 if not found
```

**API route: `POST /api/generate-audio/[date]`**
...
// Requires: article YYYY-MM-DD.md must already exist
// Calls the existing shell script — macOS only (uses `say -v Samantha`)
export async function POST(req, { params }) {
  const { date } = params;
  const script = path.join(process.env.HOME!, ".claude/scripts/article-to-audio.sh");
  await execAsync(`bash "${script}"`, {
    env: { ...process.env, TZ: "Asia/Shanghai" },
    timeout: 60000,
  });
  // Script writes LEARNING_DIR/audio/DATE.aiff and DATE.txt
  return Response.json({ ok: true });
}
```

**Note:** `article-to-audio.sh` already handles git pull, audio generation, saving `.txt`, and git push.
The launchd plist (`com.youxun.english-audio.plist`) still runs at 17:45 as a fallback —
the "Generate Audio" button in the app lets the user trigger it on demand at any time.
...
## Hooks & Scripts That Remain (do not touch)

These Claude Code hooks and scripts write data. The app reads and orchestrates them.

| Component | Script | Writes to |
|-----------|--------|-----------|
| UserPromptSubmit hook | `english-coach-hook.sh` | `vocabulary.md` (tips), session history |
| UserPromptSubmit hook | `record-english.sh` | `YYYY-MM-DD.md` (message log) |
| Stop hook | `push-english-learning.sh` | Pushes `~/.claude/english-learning/` to GitHub |
| App calls + launchd fallback | `article-to-audio.sh` | `audio/DATE.aiff`, `audio/DATE.txt` |
